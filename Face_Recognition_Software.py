...
class Login_Window:
    def __init__(self,root):
        ...
        self.root.wm_iconbitmap("logo1.jpg")
        
         # first image
        img=Image.open(r"D:\face_recognizer_project\college_photos\facial-recognition_0.jpg")
        img=img.resize((500,130),Image.ANTIALIAS)
        self.photoimg=ImageTk.PhotoImage(img)

        f_lbl=Label(self.root,image=self.photoimg)
        f_lbl.place(x=0,y=0,width=495,height=130)

        # second image
        img1=Image.open(r"D:\face_recognizer_project\college_photos\Facial_Recognition.jpg")
        ...
        # third image
        img2=Image.open(r"D:\face_recognizer_project\college_photos\smart-attendance.jpg")
        ...
        # bg image
        img3=Image.open(r"C:\Users\HP\Pictures\college_photos\background.jpg")
        img3=img3.resize((1550,800),Image.ANTIALIAS)
        self.photoimg3=ImageTk.PhotoImage(img3)

        bg_img=Label(self.root,image=self.photoimg3)
        bg_img.place(x=0,y=100,width=1550,height=800)
        
        title_lbl=Label(bg_img,text="FACE REGONITION ATTENDANCE MANAGEMENT SYSTEM",font=("times new roman",35,"bold"),bg="white",fg="red")
        ...
        img1=Image.open("college_photos/us.png")
        ...
        img2=Image.open("college_photos/us.png")
        img2=img2.resize((25,25),Image.ANTIALIAS)
        self.photoimage2=ImageTk.PhotoImage(img2)
        lblimg1=Label(image=self.photoimage2,bg="black",borderwidth=0)
        lblimg1.place(x=650,y=323,width=25,height=25)
        
        img3=Image.open("college_photos/pass.png")
        ...
        # forgetpassword
        registerbtn=Button(frame,text="Forget Password",command=self.forget_password_window,font=("times new roman",10,"bold"),borderwidth=0,relief=RIDGE,fg="white",bg="black",activeforeground="white",activebackground="black")
    ...
    def login(self):
        ...
        elif self.txtuser.get()=="sejal" and self.txtpass.get()=="ashu":
            messagebox.showinfo("Success","Welcome to place")
        else:
            conn=mysql.connector.connect(host='localhost',user='root',password='',database='face_recognizer')
            my_cursor=conn.cursor()
            my_cursor.execute("select * from face where email=%s and password=%s",(
                                                                                      self.txtuser.get(),
                                                                                      self.txtpass.get()
                                                                                )) 
    ...
    def reset_pass(self):
        if self.combo_security_Q.get()=="Select":
            ...
            conn=mysql.connector.connect(host='localhost',user='root',password='',database='face_recognizer')
            my_cursor=conn.cursor()
            qury=("select * from face where email=%s and securityQ=%s and securityA=%s")
            value=(self.txtuser.get(),self.combo_security_Q.get(),self.txt_security.get())
            my_cursor.execute(qury,value)       
            row=my_cursor.fetchone()
            if row==None:
                messagebox.showerror("Error","Please enter correct Anwer",parent=self.root2)
            else:
                query=("update face set password=%s where email=%s")
    ...
    def forget_password_window(self):
        if self.txtuser.get()=="":
            messagebox.showerror("Error","Please enter the Email adddresss to reset password")
        else:
             conn=mysql.connector.connect(host='localhost',user='root',password='',database='face_recognizer')
             my_cursor=conn.cursor()
